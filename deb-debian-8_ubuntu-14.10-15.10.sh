...
# creating package for autoplanner
rm -rf `find * | grep pyc$`
...
sudo sed -i "s/localhost/$IP/g" /etc/apache2/sites-available/autoplanner.conf
sudo sed -i "s/localhost/$IP/g" /etc/autoplanner/settings.ini
